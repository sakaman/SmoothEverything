...
2. `SmoothEverything.ControlPanel.exe`：WinUI 3/.NET 控制面板，负责编辑配置、展示诊断和按需启动同目录引擎。

拆分的目的是让输入热路径不依赖 UI 生命周期或托管 GC，同时让设置界面保持现代 Windows 体验。两个进程都以 `asInvoker` 运行。
...
自动化测试覆盖运动曲线、位移守恒、反向取消、JSON/schema、策略解析、原子配置写入、SPSC 队列、命名管道和工作线程生命周期。真实进程冒烟测试覆盖引擎启动、状态请求与正常退出；UI 运行检查覆盖四个 WinUI 页面及启动引擎交互。
